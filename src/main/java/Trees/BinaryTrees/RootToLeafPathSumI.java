...
public class RootToLeafPathSumI {
	 ...
	 */
	private static int pathSum(int type, Node<Integer> node, int partialPathSum) {
		...
		//Non Leaf
		return pathSum(type, node.left, partialPathSum) + pathSum(type, node.right, partialPathSum);
	}
}