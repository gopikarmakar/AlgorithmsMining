...
import LinkedLists.SinglyLinkedLists.MyLinkedList;
 ...
 *
 */
public class UndirectedGraph_Old {
	...
	public static void main(String[] args) {
		...
		UndirectedGraph_Old uGraph = new UndirectedGraph_Old(graph, ADJACENCY_LIST);
		uGraph.dfsPath(5);
		
		/*for(int x : uGraph.edgeTo) {
	...
	public UndirectedGraph_Old(int V) {
		this.V = V;
		this.E = 0;
		edgeTo = new int[this.V];
		marked = new boolean[this.V];
		myAdjList = new MyLinkedList[this.V];
	...
	public UndirectedGraph_Old(int[][] graph, int type) {
		this(graph.length);		
	...
	class Cycle {
		...
		public Cycle(UndirectedGraph_Old unDiGraph) {
		...
		public void dfs(UndirectedGraph_Old unDiGraph, int v, int u) {
			marked[v] = true;
